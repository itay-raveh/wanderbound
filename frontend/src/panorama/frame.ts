...
export interface PanoramaSourceGeometry {
  sourceWidth: number;
  sourceHeight: number;
  capturedFov: number;
  croppedAreaWidth?: number | null;
  croppedAreaHeight?: number | null;
  croppedAreaTop?: number | null;
}
...
export const MIN_CAPTURED_FOV = 1;
export const MAX_CAPTURED_FOV = 359;
...
function effectiveCapturedFov(
  frame: PanoramaFrameDraft,
  geometry: PanoramaSourceGeometry,
): number {
  return clamp(
    frame.capturedFov ?? geometry.capturedFov,
    MIN_CAPTURED_FOV,
    MAX_CAPTURED_FOV,
  );
}
...
function verticalSourceBounds(
  ...
  const width = geometry.croppedAreaWidth ?? geometry.sourceWidth;
  const height = geometry.croppedAreaHeight ?? geometry.sourceHeight;
  const top = geometry.croppedAreaTop ?? -height / 2;
...
export function panoramaCameraBounds(
  ...
  const capturedFov = effectiveCapturedFov(frame, geometry);
  const sourceBounds = verticalSourceBounds(geometry, capturedFov);
  const perspectiveFov = clamp(
    frame.perspectiveFov,
    MIN_PERSPECTIVE_FOV,
    maximumPerspective(capturedFov, sourceBounds, aspectRatio),
  );
  const horizontalMargin = Math.max(0, (capturedFov - perspectiveFov) / 2);
...
export function clampPanoramaFrame(
  ...
  return {
    ...
    ...(frame.capturedFov == null ? {} : { capturedFov }),
  };
}
...
export function autoFitPanoramaFrame(
  ...
  const horizontalMaximum = capturedFov - 2 * Math.abs(centered.yaw);
