...
onMounted(() => {
  ...
  placementObserver.observe(rootRef.value);
});
...
const activePanorama = computed(
  () => panorama.value?.status === "active" && panorama.value.revision != null,
);
...
function openPanoramaFrame(): void {
  updatePlacementSize();
  if (panoramaDestination.value) panoramaDialogOpen.value = true;
}
...
<template>
  ...
  >
    ...
    <PanoramaFrameDialog
      v-if="panoramaDialogOpen && albumMedia && panoramaDestination"
