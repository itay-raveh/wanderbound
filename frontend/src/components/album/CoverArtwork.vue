...
<template>
  <div class="cover-artwork relative-position">
    ...
    <MediaItem
      ...
      :class="['cover-background', { 'cover-dimmed': !isBack }]"
      :style="
        !isBack
    ...
    <div v-else-if="$slots.actions" class="album-actions" @click.stop>
      <slot name="actions" />
    </div>
...
.cover-background {
  ...
  /* rtl:ignore */
  left: calc(-1 * var(--cover-photo-left, var(--bleed)));
  width: calc(
    100% + var(--cover-photo-left, var(--bleed)) +
      var(--cover-photo-right, var(--bleed))
  );
