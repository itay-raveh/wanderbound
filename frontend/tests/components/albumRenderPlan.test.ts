...
describe("album render planning", () => {
  test("keeps a grid as one logical and physical page", () => {
    const step = makeStep({
      pages: [{ kind: "grid", media: ["one.jpg", "two.jpg"] }],
    });

    const editorItems = buildEditorItems([group([step])], new Map());
    const grid = editorItems.find((item) => item.type === "grid");
    const physicalItems = buildPhysicalRenderItems(editorItems);
    ...
    expect(physicalItems.filter((item) => item.type === "grid")).toHaveLength(1);
  });
