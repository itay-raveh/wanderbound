...
def get_session_id() -> str:
    ...
    if request is None:
        raise RuntimeError("get_session_id called outside of request context")
...
def get_trips_dir() -> Path:
    ...
    trips_dir = get_session_dir() / "trip"
...
async def extract_zip_upload(event: UploadEventArguments) -> list[str]:
    ...
    logger.info("Extracting %d MB zip upload", event.file.size() / 1024 // 1024)

    try:
        # Extract from disk
        with zipfile.ZipFile(event.file._path, "r") as zf:
            ...
            def extract_to_trip_dir() -> None:
                for file in file_list:
                    if file.startswith("trip/"):
                        zf.extract(file, session_dir)
