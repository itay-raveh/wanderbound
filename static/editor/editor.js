...
function getLayoutStorageKey() {
  // Extract trip slug from current album - use data attribute or URL
  const tripSlug = document.body.dataset.tripSlug || "default";
  return `psagen_layout_${tripSlug}`;
}
...
function saveLayoutToLocalStorage(layoutData) {
  try {
    ...
    console.log(`Layout saved to localStorage: ${key}`);
  } catch (e) {
...
function checkLocalStorageForLayout() {
  const savedLayout = getLayoutFromLocalStorage();
  if (savedLayout) {
    console.log("Found saved layout in localStorage");
    // Could show a restore prompt here if needed
  }
}
...
function initEditor() {
  ...
  // Check for saved layout in localStorage and offer restore
...
function checkAndOfferRestore() {
  ...
  // Check if current page already has edits (dirty steps)
  if (dirtySteps.size > 0) return;

  // Show restore prompt
  const shouldRestore = confirm(
    "Found saved layout in browser storage. Would you like to restore your previous edits?",
  );

  if (shouldRestore) {
    // Apply via API
    apiCall("/api/layout", { updates: savedLayout })
      .then(() => {
        alert("Layout restored! Reloading...");
        location.reload();
      })
      .catch((err) => {
        console.error("Failed to restore layout:", err);
        alert("Failed to restore layout. The saved data may be incompatible.");
      });
  }
}
...
function handleContextMenu(e) {
  ...
  document.getElementById("ctx-hide").onclick = () => {
    ...
    let hiddenContainer = mainStepPage?.querySelector(
...
window.saveAllVisibleSteps = async function () {
  ...
  try {
    ...
    // Save to localStorage for backup/restore
    saveLayoutToLocalStorage(allUpdates);
...
window.initMapLogic = function (containerId, segments, steps) {
  ...
  const loadingText =
    mapContainer.parentElement.querySelector(".map-loading-text");

  if (typeof L === "undefined") {
    if (loadingText)
      loadingText.textContent = "Error: Leaflet JS (L) is not defined.";
    return;
  }
